...
/**
 * Theme-aware button system, driven by the shadcn-style `buttonVariants`
 * (class-variance-authority) instead of the previous CSS-variable map.
 *
 * Colors/spacing now come from your Tailwind design tokens (bg-primary,
 * border-input, etc.) rather than --btn-* custom properties, so make sure
 * those tokens are defined in your theme (globals.css / tailwind config).
 */
...
export const buttonVariants = cva(
  "relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-lg border font-medium text-base outline-none transition-shadow before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-64 data-loading:select-none data-loading:text-transparent sm:text-sm [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:-mx-0.5 [&_svg]:shrink-0",
  {
    ...
    variants: {
      size: {
        default: "h-8 px-[calc(--spacing(3)-1px)] sm:h-8",
      ...
      variant: {
        default:
        "border border-primary bg-primary text-primary-foreground shadow-xs shadow-black/5 transition-colors duration-150 hover:bg-primary/90 data-pressed:bg-primary/90 *:data-[slot=button-loading-indicator]:text-primary-foreground [:disabled,:active,[data-pressed]]:shadow-none",
        secondary:
        "border border-secondary bg-secondary text-secondary-foreground shadow-xs shadow-black/5 transition-colors duration-150 hover:bg-secondary/80 data-pressed:bg-secondary/80 *:data-[slot=button-loading-indicator]:text-secondary-foreground [:disabled,:active,[data-pressed]]:shadow-none",
        destructive:
        ...
        ghost:
          "border-transparent text-foreground hover:bg-foreground/10 data-pressed:bg-accent *:data-[slot=button-loading-indicator]:text-foreground",
...
// Rendered on top of the (now-transparent) label while `loading` is true.
// `*:data-[slot=button-loading-indicator]:text-*` rules in each variant
// above color this to match that variant's text automatically.
function Spinner() {
  return (
    <svg
